//Brianna Norton & One Part Tonah w/ some help of Kalyis & Prieston
...
void draw() {
  ...
  if (!play) {
    ...
    if (enemyTimer.isFinished()) {
      enemys.add(new Enemy(int(random(80,200))));
      enemys.add(new Enemy(int(random(250,300))));
      enemyTimer.start();
    } //enemy
    for (int i = 0; i < enemys.size(); i++) {
      Enemy e = enemys.get(i);
      e.speed = frameCount/100+20;
    }
    for (int i = 0; i < enemys.size(); i++) {
      Enemy e = enemys.get(i);
      if (bob.intersect(e)) {
